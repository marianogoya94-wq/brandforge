...
export default function BrandForge() {
  ...
  const generateLogo = async (brandResult) => {
    ...
    try {
      const logoPrompts = {
        artist:   `Artistic logo icon for "${brandResult.brandName}". ${brandResult.logoDirection}. Personality: ${brandResult.personality?.join(", ")}. Colors: ${Object.values(brandResult.colors||{}).slice(0,2).map(c=>c.hex).join(" and ")}. Style: creative, expressive, bold. Isolated icon only, no text, no frames, no background elements, white background, flat vector style.`,
        business: `Corporate logo icon for "${brandResult.brandName}". ${brandResult.logoDirection}. Values: ${brandResult.personality?.join(", ")}. Colors: ${Object.values(brandResult.colors||{}).slice(0,2).map(c=>c.hex).join(" and ")}. Style: modern, professional, geometric. Isolated icon only, no text, no frames, no background elements, white background, flat vector style.`,
        commerce: `Shop logo icon for "${brandResult.brandName}". ${brandResult.logoDirection}. Brand feel: ${brandResult.personality?.join(", ")}. Colors: ${Object.values(brandResult.colors||{}).slice(0,2).map(c=>c.hex).join(" and ")}. Style: friendly, memorable, distinctive. Isolated icon only, no text, no frames, no background elements, white background, flat vector style.`,
        creator:  `Personal brand logo for "${brandResult.brandName}". ${brandResult.logoDirection}. Personality: ${brandResult.personality?.join(", ")}. Colors: ${Object.values(brandResult.colors||{}).slice(0,2).map(c=>c.hex).join(" and ")}. Style: bold, digital-native, eye-catching. Isolated icon only, no text, no frames, no background elements, white background, flat vector style.`,
      };
